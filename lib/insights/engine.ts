...
function evaluateSleepRules(sections: Sections): Insight[] {
  ...
  if (deepPct !== null && deepPct < 10) {
    insights.push({
      ...
      action: "Reduce intensity for the next recovery window",
...
function evaluateRecoveryRules(sections: Sections): Insight[] {
  ...
  if (recoveryScore !== null && recoveryScore < 40) {
    insights.push({
      ...
      action: "Cut training load and add one extra hour of sleep in the next sleep window",
  ...
  if (hrvMedian !== null && hrvMedian > 0 && hrvMedian < 20) {
    insights.push({
      ...
      severity: "critical",
      score: 86,
      action: "Stop all training and prioritize sleep and rest in the next recovery window",
